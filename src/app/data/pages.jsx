export const DoctorContent = {
    title: `Dr Konstantinos`,
    subtitle: `Principal Dentist - <span style="font-size: 16px">GDC: 281028</span>`,
    text: `<p>Dr. Konstantinos Kanaridis is a twice - Award winning Dentist qualified with Doctor of Dental
            Medicine degree (MDDr.). 
            Since qualifying, he has always maintained a keen interest in continual Postgraduate training and
            education, which has broadened his horizons and skill sets into carrying out advanced restorative
            work, smile makeovers with composite/ ceramic veneers, Invisalign, dental implants. Alongside
            general dentistry, he offers his patients the best possible care with impressive results. He has
            attended multiple Postgraduate courses including the Oral Implantology programme (PgCert.) by
            the London College of Oral Implantology (LCOI).</p>
            <p>Dr. Kanaridis has been successful in being selected to work at high end private practices among
            top rated specialists, where he gained a much deeper experience of high-quality cosmetic
            dentistry. He has offered his skills in award winning practice on the Isle of Man at the best private
            referral practice (North, UK) and in London based private dental practices on the world renowned
            Harley Street.</p>
            <p>In 2022, Dr. Kanaridis was honoured to receive an award for his case presentation as “Best Smile
            Makeover case of the year 2022”. The following year, in 2023, he once again received recognition
            for his exceptional work, this time securing the prestigious "Best Composite Bonding Case of the
            Year 2023" award.</p>
            <p>In June 2022, Dr. Kanaridis established Donti Dental Clinic in the heart of city of London, where he
            offers his dental services at a high end and relaxing premises. The clinic is installed with the latest
            dental technology, using modern equipment and techniques to give each patient the beautiful
            smile of their dreams.</p>
            <p>Dr. Kanaridis prides himself on his compassionate nature, particularly helping nervous patients
            feel at ease, and enjoys minimally invasive and ethical cosmetic dentistry. He is highly rated and
            regarded by his patients due to his great understanding of their needs and expectations.</p>`,
    textForImg: `<p>Alongside Dr. Konstantinos, we are privileged to have Sheila, who has been working as a dental
            nurse for an impressive 38 years. Her extensive experience and professionalism contribute
            significantly to the smooth running of the clinic, ensuring patients feel comfortable and well cared
            for throughout their treatment. Sheila always has a smile on her face, taking great satisfaction in
            seeing the joy on patients' faces when they experience the transformation of their smile.</p>
            <p>With her calming presence and friendly attitude, Sheila’s dedication to her role helps create a
            relaxed and positive environment for everyone. Outside of the clinic, she enjoys cycling and
            exploring new destinations in her campervan, embracing adventure and the great outdoors.</p>`,
    bannerImgSrc: '/images/doctor.jpg',
    imgAlt: 'Dt. Konstantinos Kanaridis',
    bgContnentImg: '/images/doctor_bg.jpg',
    contentImg: '/images/dental_nurse.jpeg',
    contentImgSubscription: `Sheila Divall,<br/> Dental Nurse GDC: 119846`,
}

export const DontiContent = {
    title: `Why to choose Donti Dental Clinic`,
    subtitle: `Where Innovation Meets Exceptional Care`,
    text: `<div data-aos="fade-up" data-aos-offset="-100">
            <h4>Expertise & caring team</h4>
            <p>Donti Dental Clinic is Led by Dr. Konstantinos Kanaridis, an award-winning dentist with years of
            experience in cosmetic and restorative dentistry, who focus on precision, aesthetics, and long-
            lasting results. Dr. Konstantinos is known for his meticulous attention to detail and commitment to
            patient satisfaction. Alongside him is Sheila, our highly skilled dental nurse with 38 years of
            experience, ensuring every visit is smooth, comfortable, and stress-free. Together, they create a
            warm and welcoming environment where your dental health and confidence come first.</p>
            </div>
            <div data-aos="fade-up" data-aos-delay="300" data-aos-offset="-100">
                <h4>Advanced Technology & State-of-the-Art Facilities</h4>
                <p>We use the latest dental technology and techniques to provide minimally invasive, precise, and
                effective treatments with long-lasting results. Our state-of-the-art clinic is designed for both comfort
                and innovation, offering a modern, relaxing environment where you can feel at ease while receiving
                high-quality dental care.</p>
            </div>
            <div data-aos="fade-right" data-aos-delay="500" data-aos-offset="0">
                <h4>Comprehensive Dental Services</h4>
                <p>We offer a wide range of treatments, from routine check-ups, to advanced cosmetic procedures like
                Invisalign, veneers, composite bonding, dental implants and full smile makeovers. Every treatment
                is tailored to enhance both your oral health and aesthetics.</p>
            </div>
            <div data-aos="fade-right" data-aos-delay="500" data-aos-offset="0">
                <h4>Patient-Centered Approach</h4>
                <p>We know that visiting the dentist can feel overwhelming, which is why we create a calm,
                supportive, and stress-free environment. Our team takes the time to understand your concerns,
                providing gentle, personalized care to ensure your comfort at every step.</p>
            </div>
            <div data-aos="fade-down" data-aos-delay="700" data-aos-offset="0">
                <h4>Convenient Location</h4>
                <p>Located in Central London, right outside Exit 6 of Bank Underground Station, our clinic is easily
                accessible—making top-quality dental care effortless for busy professionals and residents across the
                city.</p>
            </div>
            <div data-aos="fade-down" data-aos-delay="900" data-aos-offset="0">
                <h4>Convenient Location</h4>
                <p>Located in Central London, right outside Exit 6 of Bank Underground Station, our clinic is easily
                accessible—making top-quality dental care effortless for busy professionals and residents across the
                city.</p>
            </div>
            <div data-aos="fade-up" data-aos-delay="1200" data-aos-offset="0">
                <h4 class="o-decor o-decor--black">Free initial video consultation</h4>
                <p>Our patients come from all over the UK!</p>
                <p>We understand that traveling for dental treatment is a big commitment. That’s why we offer a free
                initial video consultation, helping you make an informed decision before visiting our London clinic</p>
            </div>
            <div data-aos="fade-up" data-aos-delay="1300" data-aos-offset="0">
                <h4>Trusted by Our Patients</h4>
                <p>Our commitment to excellence is reflected in our 5-star reviews. Patients appreciate our
                professional yet friendly approach, the quality of care we provide, and the beautiful results we
                achieve.</p>
            </div>`,
                
    bannerImgSrc: '/images/why_us.jpg',
    imgAlt: 'Donti Dental Clinic',
    bgContnentImg: '/images/bg_2.jpg'
}

export const BookingContent = {
    title: `Get started today`,
    subtitle: `Book your free virtual initial consultation`,
    text: `<h3 style="padding-bottom: 32px">Our patients come from all over the UK!</h3>
            <p>We understand that traveling for dental treatment is a big commitment. That’s why we offer a <strong>free
            initial video consultation</strong>, helping you make an informed decision before visiting our London
            clinic</p>`,
    promoText: `FREE INITIAL CONSULTATION`,
    bannerImgSrc: '/images/booking_bg.jpeg',
    imgAlt: 'Book your appointment',
}

export const PortfolioContent = {
    title: `Real Stories, Real Smiles`,
    subtitle: `Every smile has a story — see how our patients found theirs`,
    bannerImgSrc: '/images/about_2.jpg',
    interviews: [
        {
            name: 'Wiktoria',
            text: `<p>Meet Wiktoria! She wasn’t happy with the shape and shade of her teeth, and came to us looking for a fresh, confident new smile.
                With composite bonding, we refined the shape and created a brighter, more uniform appearance — in a very white shade personally selected by Wiktoria to match her vision.
                Her reaction at the end says it all!</p><p><i>Thank you, Wiktoria, for choosing us to be part of your smile story!</i></p>`,
            videoSrc: '/videos/w.mp4',
            poster: '/videos/w_poster.png'
        },
        {
            name: 'Jalyssia',
            text: `<p>Meet Jalyssia! Her main concern was the chipping on her front three teeth, which affected both the look and feel of her smile.
                With precise edge bonding on those three teeth, we restored their shape and alignment — carefully matching the shade and following the natural contours of her smile to ensure a seamless blend.
                The result? A beautifully balanced, natural-looking smile that speaks for itself!</p><p><i>Thank you, Jalyssia, for trusting us with your smile!</i></p>`,
            videoSrc: '/videos/j.mp4',
            poster: '/videos/j_poster.png'
        },
    ]
}